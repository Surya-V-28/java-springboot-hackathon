document.addEventListener('DOMContentLoaded', function () {
    ...
    loginForm.addEventListener('submit', function (event) {
        ...
        fetch('http://localhost:8080/req/login', {
            ...
            .then(response => {
                if (!response.ok) {
                    alert("error form the Javascript of the users");
            ...
            .then(data => {
               // Display the server's response (e.g., "Login successful")
                // Redirect to the desired page after login
                  alert(JSON.stringify(data) + " Good Correct part  in received for the user");
            ...
            .catch(error => {
                ...
                alert(" in the error part of the files " + error.message); // Show error message to the user
            });
    });
});